...
- `component_added(address: string, componentType: string)`
    ...
    This signal is queued by the [computer](/block/case) or
    [robot](/block/robot) when a new component is attached to it.
    ...
    [component library](/api/component) when a *primary* component
    was added / the primary component changed.

- `component_removed(address: string, componentType: string)`

    This signal is queued by the [computer](/block/case) or
    [robot](/block/robot) when a component is removed from it. The
...
- `component_available(componentType: string)`
    ...
    This signal is queued by the [component library](/api/component)
    when a *primary* component was added / the primary
...
- `component_unavailable(componentType: string)`
    ...
    This signal is queued by the [component library](/api/component)
    when a *primary* component is removed. It is generally
    preferred to use this over `component_removed`, to avoid conflicts
    with the component library.

- `term_available()`

    This signal is queued by the [term library](/api/term) when
    both a [GPU](/item/graphics_card) *and* [screen](/block/screen)
    become available in a computer. This is
...
- `term_unavailable()`
    ...
    This signal is queued by the [term library](/api/term) when
    either the primary [GPU](/item/graphics_card) or
    [screen](/block/screen) becomes unavailable in a computer. This
...
- `screen_resized(screenAddress: string, newWidth: number, newHeight: number)`
    ...
    This signal is queued by [screens](/block/screen) when their
    resolution changes, for example because it was manually set via a
    [GPU](/item/graphics_card). The address is the address of the screen
...
- `touch(screenAddress: string, x: number, y: number, button: number, playerName: string)`
    ...
    [registered as a user](/computer_users) on the computer (or
...
- `walk(screenAddress: string, x: number, y: number[, playerName: string])`
    ...
    the event. Use [gpu.getSize()](/component/gpu) to figure out
...
- `key_down(keyboardAddress: string, char: number, code: number, playerName: string)`
    ...
    This signal is queued by [keyboards](/block/keyboard) when a
...
- `clipboard(keyboardAddress: string, value: string, playerName: string)`
    ...
    is limited (can be changed in the config).
    
    `Important`: this signal is *checked*, i.e. it is only queued on a
    computer if the player that caused it is
    [registered as a user](/computer_users) on the computer (or there are
    no users registered on the computer).
...
- `redstone_changed(address: string, side: number, oldValue: number, newValue: number[, color: number])`
    ...
    when an incoming signal changes. The address is of
    the [Redstone I/O block](/block/redstone_io) or that of the
    [redstone card](/item/redstone_card) installed in the machine
    where the redstone signal was detected. The side is one of the
    [sides](/api/sides) constants and indicates on which side the
...
- `motion(address:string, relativeX:number, relativeY:number, relativeZ:number[, entityName:string])`
    ...
    Fired by the [motion sensor](/block/metion_sensor) when a
...
- `modem_message(receiverAddress: string, senderAddress: string, port: number, distance: number, ...)`
    ...
    This signal is queued by [network cards](/item/lan_card)
    ...
    [switches](/block/switch). The port is the port on which the
    ...
    specified in [modem.send()](/component/modem) or
...
- `inventory_changed(slot: number)`
    ...
    using [robot.transferTo()](/api/robot) will even trigger *four*
...
- `bus_message(protocolId: number, senderAddress: number, targetAddress: number, data: table, metadata: table)`
    ...
    - `targetAddress` is the address of the device that the messages was intended
        for (-1 for network broadcasts).
    - `data` is a table of the data that was sent.
    - `metadata` is a table of data that are unique to the device that send the
        address.
